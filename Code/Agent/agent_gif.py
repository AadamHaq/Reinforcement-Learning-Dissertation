...
class Agent:
    ...
    def generate_gif(self, frames, filename):
        # Flip and rotate frames
        flipped_frames = [np.fliplr(frame) for frame in frames]
        rotated_frames = [np.rot90(frame, k=1) for frame in flipped_frames]

        clip = ImageSequenceClip(rotated_frames, fps=1 / self.frame_duration)
        clip.write_gif(filename, fps=1 / self.frame_duration)
...
def train():
    ...
    game_numbers_to_save = [1]+[i*10 for i in range(1, 26)]
    ...
    while True:
        ...
        if done: 
            ...
            if agent.n_games in game_numbers_to_save:
                agent.generate_gif(game.frames, f"games/{agent.n_games}.gif")
            game.frames = []
